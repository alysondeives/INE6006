...
\label{questao:1b}
    ...
    Como é indicado na tabela \autoref{tb:1b}, a probabilidade de o teste $t$ rejeitar $H_0$ quando a média populacional real $\mu$ é maior que 27 aumenta conforme a média real $\mu$ se distancia de 27. Por exemplo, quando $\mu = 28$, A hipótese alternativa $H_1$ é verdadeira, no entanto o teste tem uma probabilidade de apenas 22\% de detectar essa situação a partir de uma amostra de $\texttt{n1a} = 20$ alunos, rejeitando $H_0$. Conforme a média real $\mu$ se distancia de 27, o poder do teste aumenta se aproximando de 1. Nesse caso, se a média real $\mu$ for maior que 32, praticamente qualquer amostra de 20 elementos na qual for aplicado o teste de hipótese causará a rejeição de $H_0$ e aceitação da hipótese alternativa de que $\mu > 27$.
...
\todo[inline]{Usar cores e linha contínua para $1- \beta_b$. Interpolar essa porra.}
\begin{figure}[ht]
  \centering
